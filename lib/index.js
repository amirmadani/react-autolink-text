...
function prepareElements(text) {
  ...
  this.forEach(function (match) {
    ...
    elements.push(_react2['default'].createElement(
      'a',
      { href: match.getAnchorHref(), target: _this.props.target ? _this.props.target : '_blank' },
      _this.props.linkLength && match.getAnchorText().length > _this.props.linkLength ? match.getAnchorText().substring(0, _this.props.linkLength) : match.getAnchorText()
    ));
